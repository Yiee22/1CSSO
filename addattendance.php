...
if (isset($_GET['fetch_student'])) {
    ...
    // Get student info with registration and current fines
    $query = "SELECT 
                sp.students_id,
                sp.FirstName, 
                sp.LastName, 
...
// ===== SAVE ATTENDANCE (AM/PM Login/Logout) =====
if (isset($_POST['save_attendance'])) {
    $students_id = $_POST['students_id'];
    $event_name = $_POST['event_name'];
    $event_date = $_POST['event_date'];
    $location = $_POST['location'];
    $checkpoint = $_POST['checkpoint']; // 'amLogin', 'amLogout', 'pmLogin', 'pmLogout'
    ...
    try {
        ...
        if ($result->num_rows > 0) {
            ...
            // Check if checkpoint already logged
        ...
        // ===== APPLY FINE (50 pesos per missed checkpoint) =====
        ...
        // Determine event type based on filled checkpoints
        $hasAM = !empty($attData['amLogin']) || !empty($attData['amLogout']);
        $hasPM = !empty($attData['pmLogin']) || !empty($attData['pmLogout']);
        
        // Calculate expected checkpoints and fines
        $totalFine = 0;
        $missedCheckpoints = [];
        
        if ($hasAM && $hasPM) {
            // Full day event (200 pesos total)
            if (empty($attData['amLogin'])) { $totalFine += 50; $missedCheckpoints[] = 'AM Login'; }
            if (empty($attData['amLogout'])) { $totalFine += 50; $missedCheckpoints[] = 'AM Logout'; }
            if (empty($attData['pmLogin'])) { $totalFine += 50; $missedCheckpoints[] = 'PM Login'; }
            if (empty($attData['pmLogout'])) { $totalFine += 50; $missedCheckpoints[] = 'PM Logout'; }
        } elseif ($hasAM) {
            // Morning only event (100 pesos total)
            if (empty($attData['amLogin'])) { $totalFine += 50; $missedCheckpoints[] = 'AM Login'; }
            if (empty($attData['amLogout'])) { $totalFine += 50; $missedCheckpoints[] = 'AM Logout'; }
        } elseif ($hasPM) {
            // Afternoon only event (100 pesos total)
            if (empty($attData['pmLogin'])) { $totalFine += 50; $missedCheckpoints[] = 'PM Login'; }
            if (empty($attData['pmLogout'])) { $totalFine += 50; $missedCheckpoints[] = 'PM Logout'; }
        }
        ...
        if ($totalFine > 0) {
            $checkFine = $conn->prepare("SELECT fine_id, amount FROM fines WHERE students_id = ? AND event_name = ?");
            $checkFine->bind_param("ss", $students_id, $event_name);
            $checkFine->execute();
            ...
            if ($fineResult->num_rows > 0) {
                ...
                $newAmount = $totalFine;
                $stmt = $conn->prepare("UPDATE fines SET amount = ?, fine_date = CURDATE() WHERE fine_id = ?");
                $stmt->bind_param("di", $newAmount, $fineRow['fine_id']);
                $stmt->execute();
            } else {
                // Insert new fine (only once per event)
                // This will be updated as student logs more checkpoints
            }
        }
        
        $conn->commit();
        
        // Get updated total fines
...
/* Right Section - Attendance Checkpoints */
.attendance-panel {
  background: white;
  padding: 30px;
  border-radius: 12px;
...
<div class="container">
  ...
  <div class="main-grid">
    ...
    <div class="student-input-section">
      ...
      <div id="studentInfoDisplay" class="student-info-display hidden">
        <div class="info-grid">
